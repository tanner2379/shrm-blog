...
const PostForm = props => {
  ...
  const handleSubmit = event => {
    if (userInfo.loggedIn === "LOGGED_IN") {
      ...
      axios.post(`/posts/${props.destination}`, formData, headers)
      .then(response => {
        if(response.data.status === "created"){
          setFlash({messages: ["Post Created!"], visible: true, type: "success"})
          props.history.push(`/posts/${response.data.data.slug}`)
        } else {
          setFlash({messages: response.data.errors ? response.data.errors : ["Something went wrong"], visible: true, type: "alert"})
        }
      }).catch(error => {
      ...
      setFlash({messages: ["Only the site owner can create posts"], visible: true, type: "alert"})
    }
